...
const CartPage = () => {
  ...
  const RemoveItem = async (product) => {
    try {
      ...
      const response = await axios.put(`${import.meta.env.VITE_API_URL}/api/v1/cart/${auth.user._id}`, {
        action: 'removeItem',
      ...
      <div>
        <div className="container mx-auto mt-10">
          <div className="flex shadow-md my-10">
                ...
                <>
                  < div className="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
                    <div className="flex w-2/5"> {/* product */}
                      <div className="w-20">
                        <img className="h-24" src={product?.photo?.url} alt />
                      </div>
                  ...
                  <div className="flex justify-center w-1/5">
                    <svg className="fill-current text-gray-600 w-3" viewBox="0 0 448 512" onClick={() => QuantityChange("-", product)}><path d="M416 208H32c-17.67 0-32 14.33-32 32v32c0 17.67 14.33 32 32 32h384c17.67 0 32-14.33 32-32v-32c0-17.67-14.33-32-32-32z" />
                    </svg>
                    {/* {JSON.stringify(cartItems)} */}
                    <input className="mx-2 border text-center text-bold w-12" type="text" value={product.quantity} />

                    <svg className="fill-current text-gray-600 w-3" viewBox="0 0 448 512" onClick={() => QuantityChange("+", product)}>
            ...
            <Link to="/ProductGallery" className="flex font-semibold text-indigo-600 text-sm mt-10">
          ...
          <div id="summary" className="w-1/4 px-8 py-10">
            ...
            <div className="border-t mt-8">
              ...
              <div className="py-10">
                <label htmlFor="shippingAddress" className="font-semibold block mb-2 text-sm uppercase text-gray-600">
                  Shipping Address
                </label>
               
                <p className="p-2 text-sm bg-gray-100 rounded">{auth?.user?.address}</p>
              </div>

              <div className="py-10">
                <label htmlFor="billingAddress" className="font-semibold block mb-2 text-sm uppercase text-gray-600">
                  Billing Address
                </label>
                <p className="p-2 text-sm bg-gray-100 rounded">{auth?.user?.address}</p>
              </div>
              ...
              <PaymentCheckout  deletecart={clearCart} SetLoading={SetLoading} setCartItems={setCartItems} total={TotalPrice}/>
            </div>
            <div className="border-t mt-8">
              {/* Clear Cart Button */}
              <button
